...
def gamma_correction(rgb_color, gamma=2.2):
...
if __name__ == "__main__":
    ...
    # Увеличиваем цвет поверхности K_RGB
    K_RGB = np.array([0.8, 0.8, 0.8]) # Цвет поверхности (более светлый серый, масштабированный 0-1)
    ...
    img_array = np.zeros((IMAGE_HEIGHT, IMAGE_WIDTH, 3), dtype=np.float32) # Используем float32 для промежуточных расчетов яркости
    ...
    for py in range(min_y, max_y + 1):
        for px in range(min_x, max_x + 1):
            ...
            if b0 >= -epsilon and b1 >= -epsilon and b2 >= -epsilon:
                ...
                L_RGB_float = calculate_brightness_L(PT_3D, N_triangle_normalized, V_normalized_per_pixel, LIGHT_SOURCES, K_RGB, KD, KS, KE)
    ...
    # --- Масштабирование и гамма-коррекция ---
    if max_brightness_overall > 0:
        # Масштабируем яркость так, чтобы максимальное значение стало 1.0 (или чуть меньше)
        # Это помогает избежать клиппинга при гамма-коррекции, если есть очень яркие пики
        scale_factor = 1.0 / max_brightness_overall 
        img_array_scaled = img_array * scale_factor
        
        # Применяем гамма-коррекцию
        img_array_gamma = gamma_correction(img_array_scaled * 255.0) # Переводим в 0-255 для гамма-коррекции
        
        # Ограничиваем значения до 0-255 и преобразуем в uint8
        final_img_array = np.clip(img_array_gamma, 0, 255).astype(np.uint8)
    else:
    ...
    # --- Генерация таблиц ---
    print("\n--- Вычисленные значения освещенности E1 (RGB, PT) ---")
    illuminance_E1_data = []
    ...
    for u_val in u_coords:
        ...
        for v_val in v_coords:
            if u_val + v_val <= 1.0 + 1e-6:
                ...
                E1_RGB = calculate_illuminance_E(PT_sample, LIGHT_SOURCES[0]['PL'], LIGHT_SOURCES[0]['I0_RGB'], LIGHT_SOURCES[0]['O_axis'], N_triangle_normalized)
                row_data.append(f"({int(E1_RGB[0])}, {int(E1_RGB[1])}, {int(E1_RGB[2])})")
            else:
                row_data.append("       -       ")
        illuminance_E1_data.append(row_data)

    columns = [f"u={u:.2f}" for u in u_coords]
    index = [f"v={v:.2f}" for v in v_coords]
    df_E1 = pd.DataFrame(illuminance_E1_data, index=index, columns=columns)
    print(df_E1.to_string())

    print("\n--- Вычисленные значения освещенности E2 (RGB, PT) ---")
    illuminance_E2_data = []
    for u_val in u_coords:
        row_data = []
        for v_val in v_coords:
            if u_val + v_val <= 1.0 + 1e-6:
                PT_sample = get_point_on_triangle(P0, P1, P2, u_val, v_val)
                E2_RGB = calculate_illuminance_E(PT_sample, LIGHT_SOURCES[1]['PL'], LIGHT_SOURCES[1]['I0_RGB'], LIGHT_SOURCES[1]['O_axis'], N_triangle_normalized)
                row_data.append(f"({int(E2_RGB[0])}, {int(E2_RGB[1])}, {int(E2_RGB[2])})")
            else:
                row_data.append("       -       ")
        illuminance_E2_data.append(row_data)

    df_E2 = pd.DataFrame(illuminance_E2_data, index=index, columns=columns)
    print(df_E2.to_string())

    print("\n--- Вычисленные значения яркостей L (RGB, PT) ---")
    brightness_L_data = []
    for u_val in u_coords:
        row_data = []
        for v_val in v_coords:
            if u_val + v_val <= 1.0 + 1e-6:
                ...
                L_RGB_float = calculate_brightness_L(PT_sample, N_triangle_normalized, V_normalized_per_sample, LIGHT_SOURCES, K_RGB, KD, KS, KE)
                # Отображаем значения яркости как float для большей наглядности в таблице
                row_data.append(f"({L_RGB_float[0]:.2f}, {L_RGB_float[1]:.2f}, {L_RGB_float[2]:.2f})")
            else:
                row_data.append("       -       ")
        brightness_L_data.append(row_data)

    df_L = pd.DataFrame(brightness_L_data, index=index, columns=columns)
